...
for i in range(len(results)):
	item = results[i]
	#length = len(item)
	#threshold = float(item[length-2])/float(item[length-5])
	#if (threshold > 10)
